...
export const naverMovieSearch = async (keyword) => {
  const response = await fetch(
    `https://chanoo-movie-app.herokuapp.com/api/search?keyword=${keyword}`
  );
  const json = await response.json();
...
export const getMovieImage = async (keyword, releaseDts) => {
  const url = `https://chanoo-movie-app.herokuapp.com/api/getImage?title=${keyword}&releaseDts=${releaseDts}`;
  const response = await fetch(url);
  const json = await response.json();
  if (json.errorMessage) {
    return json;
...
export const DetailMovieApi = async (title, releaseDts) => {
  const apiUrl = `https://chanoo-movie-app.herokuapp.com/api/getMovieInfo?title=${title}&releaseDts=${releaseDts}`;
